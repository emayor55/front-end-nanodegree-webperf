...
var resizePizzas = function(size) { 
  ...
  /* HOW TIME to RESIZE PIZZAS was reduced to less than 5ms: *********************************************
   *    The 'changePizzaSizes()' method was refactored to :
   * I.   remove the repeated execution of document.querySelectorAll (".randomPizzaContainer"). This method was being 
   *      called 4 times for every iteration.  Executing this ONCE, OUTSIDE of the loop, is enough. 
   * II.  replace the method querySelectorAll() with the more efficient method getElementsByClassName().
   * III. remove the unnecessary call to the determineDx() method which determines 'offsetWidth' that most likely
   *      triggers lauot. Furthermore, this method performs a roundabout calculation that is totally unnecessary 
   *      Each value of the argument 'size' maps to a specific 'newWidth' value. No need for any calculation. 
   * IV.  use the constant 100 (known number of pizza containers)  instead of 'calculating' for pcontainers.length. 
   *  *********************************************************************************************************   
   */
  
  var pcontainers=document.getElementsByClassName('randomPizzaContainer');
  ...
  function changePizzaSizes(size) {
      ...
      for ( var i = 0; i< 100; i++ ) {
    	  pcontainers[i].style.width=newWidth;
    }
  }
